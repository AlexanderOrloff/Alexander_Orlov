    ...
    if len(word) > 1:
        for i in range(0,len(word)-1): # замена и на i перед гласными
            if word[i] in target and word[i+1] in vowels:
                ...
                new_word += word[i]
        new_word += word[-1]
        if new_word[-1] not in vowels : # добавление ъ
            new_word += 'ъ'
...
def writing (text): #получает энджин # записываем html  файл с переводом на доревалюционную орфографию
...
def result():
   if request.args:
       ...
       writing(change_trigger(htmler(url)))
